...
from components.beta import BetaVAE_MLP, BetaStateVAE_MLP
from components.transforms import NormalizingFlow
from components.mlp import NLayerLeakyMLP
from components.flow import NPObsPrior
...
class SSA(pl.LightningModule):
    ...
    def __init__(
        ...
        self.obs_dis_encoder = nn.Sequential(
        ...
        self.obs_prior = NPObsPrior(self.z_dim, self.input_dim)
        ...
        for i in range(self.input_dim):
            ...
            if use_warm_start:
                ...
                print("Load pretrained spline flow", flush=True)
            self.operator_spline_list.append(spline)
        self.operator_spline_list = nn.ModuleList(self.operator_spline_list)
        ###### nonparametric identification #################################################
        ...
        self.register_buffer('obs_dist_mean', torch.zeros(self.input_dim))
        self.register_buffer('obs_dist_var', torch.eye(self.input_dim))
    ...
    @property
    def obs_dist(self):
        return D.MultivariateNormal(self.obs_dist_mean, self.obs_dist_var)
    ...
    def training_step(self, batch, batch_idx):
        ...
        _, mus, logvars, zs = self.net(x) # just learning zs
        ###### Start nonparametric identification #################################################
        distributions = self.obs_dis_encoder(x)
        x_mu = distributions[:, :self.z_dim]
        x_logvar = distributions[:, self.z_dim:]
        x_recon = self.reparameterize(x_mu, x_logvar)

        s_q_dist = D.Normal(x_mu, torch.exp(x_logvar / 2))
        log_qs = s_q_dist.log_prob(x_recon)
        
        s_residuals, s_logabsdet = self.obs_prior(zs, x_recon)
        log_ps = torch.sum(self.obs_dist.log_prob(s_residuals), dim=1) + s_logabsdet
        kld_obs = torch.sum(torch.sum(log_qs, dim=-1), dim=-1) - log_ps
        kld_obs = kld_obs.mean()
        ###### End nonparametric identification #################################################
        ...
        # VAE training
        loss = recon_loss + self.beta * kld_content + self.gamma * kld_style + self.gamma * kld_obs# + self.sigma * hsic_loss
        self.log("train_elbo_loss", loss)
        self.log("train_recon_loss", recon_loss)
        self.log("train_kld_content", kld_content)
        self.log("train_kld_style", kld_style)
        self.log("train_kld_obs", kld_obs)
        return loss

    def validation_step(self, batch, batch_idx):
        x, c, y, zst = batch['x'], batch['c'], batch['y'], batch['zst']
        batch_size, _ = x.shape
        if self.embedding_dim > 0:
            x = torch.cat([x, self.embeddings(c.squeeze().long())], dim=1)
        c = torch.squeeze(c).to(torch.int64)
        ...
        # Compute R2 of tilde zs
        zst_recon = es #zs[:,self.c_dim:]
        zst_true = zst
        if self.hz_to_z is False:
            zst_r2 = compute_r2(zst_recon, zst_true)
        else:
            zst_r2 = compute_r2(zst_true, zst_recon)

        self.log("val_mcc", mcc)
        self.log("val_zs_r2", compute_r2(mus[:,self.c_dim:], y[:,self.c_dim:]))
        self.log("val_r2", r2)  
        self.log("val_elbo_loss", loss)
        self.log("val_recon_loss", recon_loss)
        self.log("val_kld_content", kld_content)
        self.log("val_kld_style", kld_style)
        self.log("val_r2_zst", zst_r2)
