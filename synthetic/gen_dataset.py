...
def gen_da_data_ortho(
    ...
    mixedDat = np.copy(dat)   # add small scale Gaussian noise
    ...
    for l in range(Nlayer - 1):
        ...
        # apply mixing:
        mixedDat = np.dot(mixedDat, A)
    # apply general nonlinear noise at the end
    mixedDat = mixedDat + randomstate.normal(0, 0.1, dat.shape) # nonparametric
    # stratified split
    x_train, x_test, z_train, z_test, zst_train, zst_test, u_train, u_test = train_test_split(
        mixedDat, dat, zst, labels, train_size=train_size, test_size=test_size, random_state=randomstate, stratify=labels,
    )
    ...
    if save_all_datasets is True:
        ...
        for domID in range(Nsegment):
            ...
            all_datasets[domID+1] = {
                "train": {"y": z_train[train_indices], "x": x_train[train_indices], "zst": zst_train[train_indices], "c": u_train[train_indices]},
                "test": {"y": z_test[test_indices], "x": x_test[test_indices], "zst": zst_test[train_indices], "c": u_test[test_indices]}
            }
    ...
    return {"y": z_train, "x": x_train, "zst": zst_train, "c": u_train}, {"y": z_test, "x": x_test, "zst": zst_test, "c": u_test}  
